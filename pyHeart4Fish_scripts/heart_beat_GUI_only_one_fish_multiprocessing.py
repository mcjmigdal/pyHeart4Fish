    ...
    * the number of available processors is defined by the operating system
    ...
    def __init__(self, root, img, all_images):
        ...
        if os.path.isfile(os.getcwd() + "/Logo/PyHeart4Fish.ico"):
            self.root.iconbitmap(os.getcwd() + "/Logo/PyHeart4Fish.ico")
        ...
        self.b1.grid(row=0, column=1, ipady=10, ipadx=1, pady=20, padx=2, sticky=NW)
        self.play_movie_btn = ttk.Button(self.root, text="Play Movie", command=lambda: os.startfile(movie_path))
        self.play_movie_btn.grid(row=0, column=2, ipady=10, ipadx=1, pady=20, padx=2, sticky=NW)

        self.next_fish = ttk.Button(self.root, text="Next Fish", command=self.go_to_next_fish)
        self.next_fish.grid(row=0, column=3, ipady=10, ipadx=1, pady=20, padx=2, sticky=NW)
        self.exit_fish = ttk.Button(self.root, text="Exit", command=self.exit_processing)
        self.exit_fish.grid(row=0, column=4, ipady=10, ipadx=1, pady=20, padx=2, sticky=NW)
        # self.restart = ttk.Button(self.root, text="Restart Fish", command=self.restart_fish)

        # define area
        self.lb2_define_area = Label(self.root, text="2) Define Atrium and Ventricle",
                                     font=("Bahnschrift", 16))

        self.reset_bt = ttk.Button(self.root, text="Reset", command=self.reset_areas)
        self.ok_bt = ttk.Button(self.root, text="OK", command=self.start_progressing)
        self.help_bt = ttk.Button(self.root, text="Help", command=self.need_help)
        ...
        self.canvas1.grid(row=1, column=0, sticky=NW, columnspan=20)

        self.image_object = ImageEnhance.Contrast(self.image).enhance(2.4)
    ...
    @staticmethod
    def exit_processing():
        with open(main_output_path + "/status.txt", "w") as file:
            file.write("stop all")
            # if os.path.isfile(main_output_path + "/config_file.json"):
            # os.remove(main_output_path + "/config_file.json")
            if os.path.isfile(main_output_path + "/config_file_processed.json"):
                os.remove(main_output_path + "/config_file_processed.json")
        if os.path.isfile(main_output_path + "/status.txt"):
            os.remove(main_output_path + "/status.txt")
        sys.exit()
    ...
    def define_areas(self):
        ...
        self.s.destroy()
        self.canvas1.delete('all')
        self.lb2_define_area.grid(row=0, column=0, pady=2, padx=5, sticky=E)
        self.ok_bt.grid(row=0, column=1, ipady=10, ipadx=0, pady=20, padx=1, sticky=NW)
        self.reset_bt.grid(row=0, column=2, ipady=10, ipadx=0, pady=20, padx=1, sticky=NW)
        self.help_bt.grid(row=0, column=3, ipady=10, ipadx=0, pady=20, padx=1, sticky=NW)
        self.next_fish.grid(row=0, column=4, ipady=10, ipadx=0, pady=20, padx=1, sticky=NW)
        self.rotated_np_img = self.image.rotate(360 - float(self.degrees_out), PIL.Image.Resampling.BICUBIC, expand=0)
    ...
    def start_progressing(self):
        ...
        if not os.path.isfile(main_output_path + "/config_file_processed.json"):
            messagebox.showerror("Area definition incomplete!", "Please define atrium and ventricle areas")
            self.reset_areas()
            return

        with open(main_output_path + "/config_file_processed.json", "r") as config_file_:
        ...
        self.lb2_define_area.destroy()
        self.reset_bt.destroy()
        self.ok_bt.destroy()
        self.help_bt.destroy()
        self.define_areas.canvas.destroy()
        self.canvas1.delete('all')
        ...
        self.next_fish.grid(row=0, column=2, ipady=5, ipadx=1, pady=20, padx=1, sticky=NS)
        self.exit_fish.grid(row=0, column=3, ipady=5, ipadx=1, pady=20, padx=1, sticky=NS)
        ...
        background1 = np.max(img_temp2[:, :int(0.85 * min(x_min, x_min2))])
        if int(1.14 * max(x_max, x_max2)) < len(img_temp2[0]):
            background2 = np.max(img_temp2[:, int(1.14 * max(x_max, x_max2)):])
        else:
            background2 = 1
        background3 = np.max(img_temp2[:int(0.85 * min(y_min, y_min2)), :])
        if int(1.14 * max(y_max, y_max2)) < len(img_temp2):
            background4 = np.max(img_temp2[int(1.14 * max(y_max, y_max2)):, :])
        else:
            background4 = 1

        print("Take max from temporary thresholds: ", background1, background2, background3, background4, threshold)
        threshold = int(np.max([background1, background2, background3, background4, threshold]) * 1.1)

        print("Threshold: ", threshold)
        configs_["threshold"] = threshold
        with open(main_output_path + "/config_file_processed.json", "w") as config_file_:
        ...
        def start_progress_bar():
            ...
            img_temp_tk = img_temp_tk.resize((int(self.w/self.resize_factor), int(self.h/self.resize_factor)))
...
class DefineArea:
    def __init__(self, root, new_image):
        ...
        self.canvas = Canvas(self.root, cursor="heart",  height=self.display_height, width=self.display_width,
                             background="black")
        self.canvas.grid(row=1, column=0, sticky=NW, columnspan=20)
        self.new_image = ImageEnhance.Contrast(self.new_image).enhance(2.4)
    ...
    def on_button_release(self, event):
        ...
        if self.color == "blue":
            with open(main_output_path + "/config_file.json", "r") as config_file_:
                configs_ = json.load(config_file_)
...
def process_other_images_and_plot_hearts(image, param):
    ...
    * process image and find atrium / ventricle
    ...
    if len(img_temp) < 200:
        resize_x = 0.5
    atr_box_size = int(np.mean([(areas[1][1] - areas[0][1]), (areas[1][0] - areas[0][0])]) * 0.36 * resize_x)
    ...
    kernel2 = np.ones((ks, ks), np.uint8)
    if len(img_temp) > 300:
        img_temp = cv2.erode(img_temp, kernel2, iterations=2)
        img_temp = cv2.dilate(img_temp, kernel2, iterations=2)
    else:
    ...
    a_ = img_temp[
         atrium_inner_box_y: atrium_inner_box_y + atr_box_size,
         atrium_inner_box_x: atrium_inner_box_x + atr_box_size
         ]
    atr_intensity = np.sum(a_)
    v_ = img_temp[
         ventr_inner_box_y: ventr_inner_box_y + ventrl_box_size,
         ventr_inner_box_x: ventr_inner_box_x + ventrl_box_size
         ]
    ventr_intensity = np.sum(v_)
...
def process_raw_data_and_plot_heart_beat_curve():
    ...
    * calculates and extracts:
    ...
    # heart params
    value_dict["max_dilated_atrium_area (µm^2)"] = max_atrium
    value_dict["max_contracted_atrium_area (µm^2)"] = min_atrium
    ...
    value_dict["max_dilated_ventricle_area (µm^2)"] = max_ventricle
    value_dict["max_contracted_ventricle_area (µm^2)"] = min_ventricle
    ...
    value_dict["conduction_score (norm < 0.5)"] = conduction_score
    # fit raw heartbeat data
    ...
    plt.text(x=max(x_values) * 1.1, y=np.mean(atrium_values), rotation=270,
             s=fish_heart_file, fontdict={"size": 8}, ha="center", va="center")
    ...
    plt.savefig(main_output_path + "/%s_frequencies_fps%s.pdf" % (fish_heart_file, frames_per_second))
...
def create_gifs_from_images():
    ...
    if len(image_container) != 0:
        ...
        img.save(fp=main_output_path + "/" + "%s_heart_beats_fps%s.gif" % (fish_heart_file, frames_per_second),
...
if __name__ == '__main__':
    ...
    with open(main_output_path + "/config_file_processed.json", "r") as config_file:
